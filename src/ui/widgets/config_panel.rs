...
use crate::{ui::{key_types::KEY_TYPES, key_row::KeyRow, state::State}, key::{KeyType, Key}};
   ...
   ui.horizontal(|ui| {
                ...
                if ui.button("Генерувати").clicked() {
                    ...
                    match KEY_TYPES.get(state.key_type_id).unwrap().value {
                        ...
                        KeyType::CustomKey(_, _) => {
                            key_rows =
                                Key::generate(KeyType::CustomKey(&state.key_alphabet, state.key_length), state.count)
                                    .iter()
                                    .map(|key| -> KeyRow {
                                        return KeyRow {
                                            value: key.clone(),
